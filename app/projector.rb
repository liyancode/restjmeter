      ...
      if method_type.upcase=='GET'
        ...
        test name:test_id do
          threads count:body_hash["ThreadProperties"]["Number_of_Threads"].to_i,
            ...
            aggregate_report
            visit name:"#{body_hash["API"]["Http_or_Https"]}://#{body_hash["API"]["ServerName_or_IP"]}#{body_hash["API"]["Path"].split("?")[0]}_#{method_type.upcase}".gsub!('$',''),
        ...
        test name:test_id do
          threads count:body_hash["ThreadProperties"]["Number_of_Threads"].to_i,
            ...
            aggregate_report
            post name:"#{body_hash["API"]["Http_or_Https"]}://#{body_hash["API"]["ServerName_or_IP"]}#{body_hash["API"]["Path"].split("?")[0]}_#{method_type.upcase}".gsub!('$',''),
                 url:"#{body_hash["API"]["ServerName_or_IP"]}",
                 protocol:"#{body_hash["API"]["Http_or_Https"]}",
                 method:"#{method_type}",
                 path: "#{body_hash["API"]["Path"]}#{body_hash["API"]["Parameters"]}",
